...
const News = () => {
    return (
        <div className='py-16'><h2 className='text-3xl font-semibold mb-6'>news</h2>
            <Swiper
                slidesPerView={2}
                spaceBetween={30}
                navigation={true}
            ...
            >
                ...
                {
                    news.map((item, index) => (<SwiperSlide key={index}><div className='flex flex-col sm:flex-row sm:justify-between  items-center
                    gap-12'>
                        <div className='py-4'><Link to="/"><h3 className='text-lg font-medium hover:text-blue-600 mb-4'>{item.title}</h3></Link>
                            <div className='w-12 h-[4px] bg-black mb-5'></div>
                            <p className='text-sm text-gray-600'>{item.description}</p></div>
